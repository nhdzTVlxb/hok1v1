...
import os
from agent_ppo.model.model import Model, load_pretrain_single_head
...
class Agent(BaseAgent):
    def __init__(self, agent_type="player", device=None, logger=None, monitor=None):
        ...
        self.hero_rearrange = ListRearrange()
    ...
    def _model_inference(self, list_obs_data):
        # 先基于英雄索引对obs进行重排, 便于使用不同的输出头网络预测
        hero_idx = [int(x.feature[0])+1 for x in list_obs_data]  # 获得每个obs操控的英雄编号
        self.hero_rearrange.update(hero_idx)
        list_obs_data = self.hero_rearrange.forward(list_obs_data)  # 正向重排
        hero_split_nums = self.hero_rearrange.split_nums
        ...
        format_inputs = [feature_vec, lstm_hidden_state, lstm_cell_state, hero_split_nums]
        ...
        list_act_data = self.hero_rearrange.inverse(list_act_data)  # 反向重排
    ...
    def learn(self, list_sample_data):
        # 先基于英雄索引对obs进行重排, 便于使用不同的输出头网络预测 (与_model_inference不同的是, 这里无需返回到初始顺序)
        # SampleData的npdata中按照[obs_dim*B*L, ...]保存, 由于一个序列中一定是控制同一个英雄,
        # 因此考虑第一个obs信息即可判断整个SampleData的英雄编号
        hero_idx = [int(x.npdata[0])+1 for x in list_sample_data]  # 获得每个obs操控的英雄编号
        # print(f"{'='*10}[DEBUG] {hero_idx=}{'='*10}")
        self.hero_rearrange.update(hero_idx)
        list_sample_data = self.hero_rearrange.forward(list_sample_data)
        hero_split_nums = self.hero_rearrange.split_nums

        return self.algorithm.learn(list_sample_data, hero_split_nums)
    ...
    def load_model(self, path=None, id="1"):
        ...
        if self.cur_model_name == model_file_path:
            ...
            try:
                ...
                load_pretrain_single_head(self.model, load_dict)
                self.logger.info("load model from single head to multi head successfully")
