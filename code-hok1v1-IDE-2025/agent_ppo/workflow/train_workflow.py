...
class EpisodeRunner:
    ...
    def run_episodes(self):
        ...
        while True:
            ...
            # Get more eval opponents; 114608 is pt1_ft1; 118195 is pt1_ft2; 131636 is pt4_ft2_chec1
            usr_conf["episode"]["eval_opponent_type"] = random.choice(["114608", "118195","131636"])
            ...
            while True:
                ...
                for index, (do_predict, do_sample, agent) in enumerate(
                    ...
                    if do_predict:
                        ...
                        if agent.at_low_health:
                            # Log the action when at low health
                            self.logger.info(f"Agent {index} at low health, forcing retreat action {actions[index]}")
                ...
                for i, (do_sample, agent) in enumerate(zip(self.do_samples, self.agents)):
                    if do_sample:
                        ...
                        for key, value in reward.items():
                            self.logger.info(f"reward for {key}: {value}")
                        reward_sum_list[i] += reward["reward_sum"]
                ...
                if is_gameover:
                    ...
                    for hero in observation[0]["frame_state"]["hero_states"]:
                        sk2_used += hero["skill_state"]["slot_states"][3]["usedTimes"]
                        sk2_hit += hero["skill_state"]["slot_states"][3]["hitHeroTimes"]
                        sk2_hit_rate = sk2_hit / sk2_used if sk2_used > 0 else 0
                        sk3_used += hero["skill_state"]["slot_states"][4]["usedTimes"]
                        sk3_hit += hero["skill_state"]["slot_states"][4]["hitHeroTimes"]
                        sk3_hit_rate = sk3_hit / sk3_used if sk3_used > 0 else 0
