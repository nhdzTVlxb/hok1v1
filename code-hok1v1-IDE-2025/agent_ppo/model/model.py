...
class Model(nn.Module):
    def __init__(self):
        ...
        fc_concat_dim_list = [self.feature_dim, 512, self.lstm_unit_size]
    ...
    def forward(self, data_list, inference=False):
        ...
        # Ensure tensors are contiguous before unsqueeze and LSTM operations
        # 确保张量在unsqueeze和LSTM操作之前是连续的
        h_0 = lstm_hidden_init.unsqueeze(0).contiguous()
        c_0 = lstm_cell_init.unsqueeze(0).contiguous()

        if inference:
            # For inference, input is [N, H_in]. Add a sequence dimension (L=1).
            # 推理时，输入是 [N, H_in]，增加一个序列维度 (L=1)
            lstm_input = fc_public_result.unsqueeze(1).contiguous()
            lstm_output, (h_n, c_n) = self.lstm(lstm_input, (h_0, c_0))
            # Squeeze the sequence dimension back out. Output is [N, H_out].
            # 挤压掉序列维度，输出为 [N, H_out]
            lstm_output_squeezed = lstm_output.squeeze(1)
        else:
            # For training, input is [N, L, H_in] where L=16.
            # 训练时，输入是 [N, L, H_in]，L=16
            lstm_input = fc_public_result.contiguous()
            lstm_output, (h_n, c_n) = self.lstm(lstm_input, (h_0, c_0))
            # Reshape output to [N*L, H_out] for the loss function.
            # 为损失函数将输出重塑为 [N*L, H_out]
            lstm_output_squeezed = lstm_output.reshape(-1, self.lstm_unit_size)
        ...
        # output label - Now fed from LSTM output
        # 输出标签
        for label_index, label_dim in enumerate(self.label_size_list[:]):
            label_mlp_out = self.label_mlp["hero_label{0}_mlp".format(label_index)](lstm_output_squeezed)
            result_list.append(label_mlp_out)

        # output value - Now fed from LSTM output
        # 输出价值
        value_result = self.value_mlp(lstm_output_squeezed)
