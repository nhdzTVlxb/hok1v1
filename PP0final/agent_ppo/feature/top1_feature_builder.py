...
from agent_ppo.feature.layout import (
    TARGET_ENEMY_HERO,
    TARGET_ENEMY_SOLDIER_COUNT,
    TARGET_ENEMY_SOLDIER_START,
    TARGET_ENEMY_TOWER,
    TARGET_MONSTER,
    TARGET_NONE,
    TARGET_SELF_HERO,
...
class Top1FeatureBuilder:
    ...
    def build_observation(self, observation):
        ...
        feature_blocks = [
            ("hero_frd", self._hero_features(our_hero, our_hero, main_camp, enemy_tower, always_visible=True)),
            ("hero_emy", self._enemy_hero_features(our_hero, enemy_hero, main_camp)),
            ("skill", self._skill_features(our_hero)),
            ("tower", self._tower_features(our_hero, our_tower, enemy_tower)),
            ("soldier_summary", self._soldier_features(our_hero, our_soldiers, enemy_soldiers, main_camp)),
            ("monster", self._monster_features(our_hero, monsters)),
            ("cake", self._cake_features(our_hero, cakes)),
            ("unit_slots", self._unit_slot_features(our_hero, our_soldiers, enemy_soldiers, monsters, main_camp)),
            ("projectile_slots", self._projectile_features(our_hero, bullets, main_camp)),
            ("combat_risk", self._combat_risk_features(our_hero, enemy_hero, our_tower, enemy_tower, main_camp)),
            ("objective", self._objective_features(frame_state, our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, monsters, cakes, main_camp)),
            ("global", self._global_features(frame_state, our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers)),
            ("status_macro", self._status_macro_features(frame_state, our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, cakes, bullets, main_camp)),
            ("strategic_macro", self._strategic_macro_features(frame_state, our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, monsters, cakes, bullets, main_camp)),
            ("legal_summary", self._legal_features(observation)),
            ("ability_control", self._ability_control_features(our_hero, enemy_hero, main_camp)),
            ("lane_entity", self._lane_entity_features(our_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, main_camp)),
            ("tower_wave", self._tower_wave_features(our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, cakes, main_camp)),
            ("projectile_detail", self._projectile_detail_features(our_hero, bullets, enemy_hero, main_camp)),
            ("position_bucket", self._position_bucket_features(our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers)),
            ("buff_mark_detail", self._buff_mark_detail_features(our_hero, enemy_hero, main_camp)),
            ("economy_phase", self._economy_phase_features(our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, monsters, cakes, main_camp)),
            ("hero_attr_detail", self._hero_attr_detail_features(our_hero, enemy_hero, main_camp)),
            ("skill_detail", self._skill_detail_features(our_hero, enemy_hero, main_camp)),
        ]

        target_features = self._target_entity_features(our_hero, enemy_hero, enemy_tower, enemy_soldiers, monsters, main_camp)
        feature = assemble_feature_blocks(feature_blocks) + self._fit(target_features, Config.TARGET_FEATURE_DIM)
    ...
    def _hero_attr_detail_features(self, our_hero, enemy_hero, main_camp):
        enemy_visible = enemy_hero is not None and is_visible_to(enemy_hero, main_camp) and hp_ratio(enemy_hero) > 0
        ...
        def pack(hero):
            ...
            return self._fit(
                [
                    ratio(get_any(hero, "phy_atk", default=0), 1200),
                    ratio(get_any(hero, "phy_def", default=0), 1200),
                    ratio(get_any(hero, "mgc_atk", default=0), 1200),
                    ratio(get_any(hero, "mgc_def", default=0), 1200),
                    ratio(get_any(hero, "mov_spd", default=0), 2000),
                    ratio(get_any(hero, "atk_spd", default=0), 2000),
                    ratio(get_any(hero, "hp_recover", default=0), 1000),
                    ratio(get_any(hero, "ep_recover", default=0), 1000),
                    ratio(get_any(hero, "phy_armor_hurt", default=0), 1000),
                    ratio(get_any(hero, "mgc_armor_hurt", default=0), 1000),
                    ratio(get_any(hero, "crit_rate", default=0), 10000),
                    ratio(get_any(hero, "crit_effe", default=0), 10000),
                    ratio(get_any(hero, "phy_vamp", default=0), 10000),
                    ratio(get_any(hero, "mgc_vamp", default=0), 10000),
                    ratio(get_any(hero, "cd_reduce", default=0), 10000),
                    ratio(get_any(hero, "ctrl_reduce", default=0), 10000),
                    ratio(get_any(hero, "exp", default=0), 30000),
                    ratio(get_any(hero, "money", default=0), 3000),
                    ratio(get_any(hero, "money_cnt", default=0), 15000),
                    ratio(get_any(hero, "total_hurt", default=0), 120000),
                    ratio(get_any(hero, "total_hurt_to_hero", default=0), 80000),
                    ratio(get_any(hero, "total_be_hurt_by_hero", default=0), 80000),
                    ratio(get_any(hero, "kill_income", default=0), 800),
                    ratio(get_any(hero, "sight_area", default=0), 40000),
                    ratio(len(((hero.get("equip_state", {}) or {}).get("equips", []) or [])), 6),
                    ratio(len(((hero.get("buff_state", {}) or {}).get("buff_skills", []) or [])), 12),
                    ratio(len(((hero.get("buff_state", {}) or {}).get("buff_marks", []) or [])), 12),
                    float(get_any(hero, "attack_target", default=0) not in (0, None)),
                ],
                32,
            )

        return pack(our_hero) + (pack(enemy_hero) if enemy_visible else [0.0] * 32)

    def _skill_detail_features(self, our_hero, enemy_hero, main_camp):
        enemy_visible = enemy_hero is not None and is_visible_to(enemy_hero, main_camp) and hp_ratio(enemy_hero) > 0
        ...
        def pack(hero):
            ...
            for slot in slots:
                ...
                cd_max = max(float(get_any(slot, "cooldown_max", default=1) or 1), 1.0)
                out += [
                    ratio(get_any(slot, "slot_type", default=0), 7),
                    ratio(int_value(get_any(slot, "configId", "config_id", default=0)) % 100000, 100000),
                    ratio(get_any(slot, "nextConfigID", "next_config_id", default=0), 100000),
                    ratio(get_any(slot, "level", default=0), 6),
                    float(bool(get_any(slot, "usable", default=False))),
                    clip(cd / cd_max, 0.0, 1.0),
                    ratio(get_any(slot, "usedTimes", "used_times", default=0), 120),
                    ratio(get_any(slot, "hitHeroTimes", "hit_hero_times", default=0), 30),
                ]
            return self._fit(out, 56)

        return pack(our_hero) + (pack(enemy_hero) if enemy_visible else [0.0] * 56)
    ...
    def _target_entity_features(self, our_hero, enemy_hero, enemy_tower, enemy_soldiers, monsters, main_camp):
        out = []
        out += self._target_entity_slot(our_hero, None, TARGET_NONE, main_camp)
        out += self._target_entity_slot(our_hero, enemy_hero, TARGET_ENEMY_HERO, main_camp)
        out += self._target_entity_slot(our_hero, our_hero, TARGET_SELF_HERO, main_camp)
        soldiers = [s for s in enemy_soldiers if is_visible_to(s, main_camp)]
        if our_hero:
            soldiers.sort(key=runtime_id)
        for index, soldier in enumerate(soldiers[:TARGET_ENEMY_SOLDIER_COUNT]):
            out += self._target_entity_slot(our_hero, soldier, TARGET_ENEMY_SOLDIER_START + index, main_camp, tower=enemy_tower)
        while len(out) < TARGET_ENEMY_TOWER * Config.TARGET_ENTITY_DIM:
            out += [0.0] * 32
        out += self._target_entity_slot(our_hero, enemy_tower, TARGET_ENEMY_TOWER, main_camp)
        visible_monsters = [m for m in monsters if not (abs(actor_pos(m)[0]) >= UNSEEN_PADDING or abs(actor_pos(m)[1]) >= UNSEEN_PADDING)]
        if our_hero:
            visible_monsters.sort(key=lambda m: distance(our_hero, m))
        out += self._target_entity_slot(our_hero, visible_monsters[0] if visible_monsters else None, TARGET_MONSTER, main_camp)
