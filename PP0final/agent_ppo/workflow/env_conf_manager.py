...
class EnvConfManager:
    ...
    def _read_usr_conf(self):
        if tomllib is not None:
            with open(self.config_path, "rb") as file_obj:
                conf = tomllib.load(file_obj)
        else:
            with open(self.config_path, "r", encoding="utf-8") as file_obj:
                conf = self._parse_minimal_toml(file_obj.read())
    ...
    def _parse_minimal_toml(self, text):
        ...
        while idx < len(lines):
            ...
            if raw == "[[lineups.blue_camp]]":
                section = {}
                conf["lineups"]["blue_camp"].append(section)
                continue
            if raw == "[[lineups.red_camp]]":
                section = {}
                conf["lineups"]["red_camp"].append(section)
                continue
