...
from agent_ppo.utils_action_audit import ActionAuditDumper
...
class EpisodeRunner:
    def __init__(self, env, agents, logger, monitor, env_conf_manager, lineup_iterator):
        ...
        self.action_auditor = ActionAuditDumper(logger)
    ...
    def run_episodes(self):
        ...
        while True:
            ...
            for agent_idx, agent in enumerate(self.agents):
                ...
                self.obs_dumper.dump(self.episode_cnt, agent_idx, frame_no, observation[str(agent_idx)], usr_conf=usr_conf)
            ...
            while True:
                ...
                for index, (do_predict, do_sample, agent) in enumerate(
                    ...
                    if do_predict:
                        ...
                        self.action_auditor.dump(
                            self.episode_cnt,
                            index,
                            frame_no,
                            observation[str(index)],
                            actions[index],
                            getattr(agent, "last_action_source", "unknown"),
                            usr_conf=usr_conf,
                            is_eval=is_eval,
                        ...
                        self.diagnostics.observe(index, observation[str(index)], actions[index])
                ...
                for agent_idx in range(self.agent_num):
                    self.obs_dumper.dump(
                        ...
                        agent_idx,
                        frame_no,
                        observation[str(agent_idx)],
                        usr_conf=usr_conf,
                        extra_info=extra_info,
                        env_meta={"terminated": terminated, "truncated": truncated},
                    )
                ...
                train_test_max_frame = int(os.environ.get("TRAIN_TEST_MAX_FRAME", "6000") or 6000)
                is_gameover = terminated or truncated or (is_train_test and frame_no >= train_test_max_frame)
                if is_gameover:
                    for agent_idx, agent in enumerate(self.agents):
                        agent.dump_feature_audit(observation[str(agent_idx)], force=True)
                        self.obs_dumper.dump(
                            self.episode_cnt,
                            agent_idx,
                            frame_no,
                            ...
                            usr_conf=usr_conf,
                            extra_info=extra_info,
                            env_meta={"terminated": terminated, "truncated": truncated, "final": True},
                        )
                    self.logger.info(
                    ...
                    if now - self.last_report_monitor_time >= 60:
                        ...
                        if self.monitor:
                            ...
                            metrics = self.diagnostics.episode_metrics(monitor_side)
