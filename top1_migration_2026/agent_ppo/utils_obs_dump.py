...
class ObsDumper:
    def __init__(self, logger=None):
        ...
        self.every_n = int(config.get("DUMP_OBS_EVERY_N", 0) or 0)
        self.start_frame = int(config.get("DUMP_OBS_START_FRAME", 0) or 0)
        self.end_frame = int(config.get("DUMP_OBS_END_FRAME", 0) or 0)
        ...
        if self.enabled:
            ...
            self._log(
                f"DUMP_OBS enabled, frames={sorted(self.frames)}, every_n={self.every_n}, "
                f"range=[{self.start_frame}, {self.end_frame}], dir={self.output_dir}"
            )
    ...
    def dump(self, episode, agent_idx, frame_no, observation, usr_conf=None, extra_info=None, env_meta=None):
        ...
        if self.every_n > 0:
            range_end = self.end_frame if self.end_frame > 0 else current_frame
            dense_start = max(last_frame + 1, self.start_frame)
            dense_end = min(current_frame, range_end)
            if dense_start <= dense_end:
                first = dense_start + ((self.every_n - dense_start % self.every_n) % self.every_n)
                target_frames.extend(range(first, dense_end + 1, self.every_n))
