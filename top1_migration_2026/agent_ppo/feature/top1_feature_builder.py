...
class Top1FeatureBuilder:
    ...
    def build_observation(self, observation):
        ...
        # Keep this block reserved until the 2026 debug dumps confirm bullet
        # fields.  The available local examples are 2025 dumps and may not be
        # schema-compatible with the current environment.
        feature += [0.0] * 160
    ...
    def _strategic_macro_features(self, frame_state, our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, monsters, cakes, bullets, main_camp):
        ...
        summon_ready = self._slot_ready(our_hero, 6)
        flash_ready = float(summon_id == FLASH_SKILL_ID and summon_ready)
        ...
        direnjie_flash_ult_setup = float(
            int(get_any(our_hero, "config_id", "configId", default=0) or 0) == 133
            and flash_ready
            and skill3_ready
            and enemy_visible
            and 7600.0 < enemy_dist <= 13500.0
            and (fight_score >= 0.35 or enemy_hp <= 0.45)
        )
        ...
        out = [
            ...
            flash_ready,
            berserk_ready,
            float(summon_id == FLASH_SKILL_ID),
    ...
    def _fight_score(self, our_hero, enemy_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, main_camp):
        ...
        if self._slot_ready(our_hero, 6):
            score += 0.16 if summon_id == BERSERK_SKILL_ID else 0.10 if summon_id == FLASH_SKILL_ID else 0.0
    ...
    def _nearest_grass_distance(self, hero):
        if not hero:
            return MAP_SCALE
        hero_pos = actor_pos(hero)
        return min(math.dist(hero_pos, point) for point in GRASS_POINTS)
    ...
    def _lane_entity_features(self, our_hero, our_tower, enemy_tower, our_soldiers, enemy_soldiers, main_camp):
        ...
        our.sort(key=lambda s: distance(our_hero, s))
        enemy.sort(key=lambda s: distance(our_hero, s))
        out = []
    ...
    def _target_entity_features(self, our_hero, enemy_hero, enemy_tower, enemy_soldiers, monsters, main_camp):
        ...
        if our_hero:
            soldiers.sort(key=lambda s: distance(our_hero, s))
