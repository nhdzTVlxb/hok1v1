...
def is_soldier_actor(actor):
    ...
    if is_neutral_camp(get_any(actor or {}, "camp", default=None)):
        return False
    return at == 1 and st in (1, 11, "ACTOR_SUB_SOLDIER")
...
def is_monster_actor(actor):
    ...
    return (
        at in (3, "ACTOR_TYPE_MONSTER")
        or st in ("ACTOR_SUB_MONSTER", "ACTOR_SUB_NEUTRAL_MONSTER")
        or (is_neutral_camp(get_any(actor or {}, "camp", default=None)) and at == 1 and st == 0)
    )
...
class GameRewardManager:
    ...
    def calculate_monster_contest(self, frame_data, main_hero, enemy_hero, enemy_tower):
        ...
        if money < getattr(GameConfig, "TOWER_EARLY_PUSH_MONEY", 5500):
            scale = 1.0
    ...
    def push_money_scale(self, main_hero):
        ...
        if money < getattr(GameConfig, "TOWER_EARLY_PUSH_MONEY", 5500):
            return 0.15
        if money < getattr(GameConfig, "TOWER_FULL_PUSH_MONEY", 6000):
            return 0.45
        return 1.0
    ...
    def calculate_finish_push(self, camp, frame_data, main_hero, enemy_hero, enemy_tower):
        if not main_hero or not enemy_tower:
            return 0.0
        if int(get_any(main_hero, "level", default=1) or 1) < getattr(GameConfig, "POST_FIGHT_PUSH_LEVEL", 9):
            return 0.0
        if float(get_any(main_hero, "money_cnt", "money", default=0) or 0) <= getattr(GameConfig, "POST_FIGHT_PUSH_MONEY", 6000):
            return 0.0
        if hp_ratio(main_hero) <= getattr(GameConfig, "POST_FIGHT_PUSH_HP_THRESHOLD_LATE", 0.05):
            return 0.0
    ...
    def cake_side(self, cake):
        ...
        if x == 0 and z == 0:
            return 0
        # Blue-side objects are on the negative diagonal in the 2026 1v1 map.
        return 1 if x + z < 0 else 2
    ...
    def calculate_grass_ambush(self, frame_data, main_hero, enemy_hero, enemy_tower):
        ...
        if enemy_hero and hp_ratio(enemy_hero) > 0 and not self.in_enemy_tower_range(main_hero, enemy_tower):
            if math.dist(pos(main_hero), pos(enemy_hero)) <= float(get_any(main_hero, "attack_range", default=0) or 0) + 6000.0:
                return 1.0
        return 0.4
