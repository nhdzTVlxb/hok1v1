...
class GameRewardManager:
    ...
    def set_cur_calc_frame_vec(self, calc_frame_map, frame_data, camp):
        ...
        for reward_name, reward_struct in calc_frame_map.items():
            ...
            elif reward_name == "luban_lane_suppress":
                reward_struct.cur_frame_value = self.calculate_luban_lane_suppress(frame_data, main_hero, enemy_hero, enemy_tower)
            ...
            elif reward_name == "early_bad_fight":
                reward_struct.cur_frame_value = self.calculate_early_bad_fight(frame_data, main_hero, enemy_hero, main_tower, enemy_tower)
    ...
    def calculate_luban_lane_suppress(self, frame_data, main_hero, enemy_hero, enemy_tower):
        frame_no = get_any(frame_data, "frame_no", "frameNo", default=0) or 0
        if frame_no > getattr(GameConfig, "LUBAN_VS_DI_EARLY_FRAME", 1800):
            return 0.0
        if not main_hero or int(get_any(main_hero, "config_id", "configId", default=0) or 0) != 112:
            return 0.0
        if not enemy_hero or int(get_any(enemy_hero, "config_id", "configId", default=0) or 0) != 133:
            return 0.0
        main_camp = camp_id(main_hero.get("camp"))
        hero_pos = pos(main_hero)
        enemy_pos = pos(enemy_hero)
        enemy_visible = hp_ratio(enemy_hero) > 0 and abs(enemy_pos[0]) < 100000 and abs(enemy_pos[1]) < 100000
        enemy_soldiers = [
            npc
            for npc in frame_data.get("npc_states", []) or []
            if is_soldier_actor(npc)
            and camp_id(npc.get("camp")) != main_camp
            and float(get_any(npc, "hp", default=0) or 0) > 0
            and math.dist(hero_pos, pos(npc)) <= 10000.0
        ]
        our_soldiers, their_soldiers = self.lane_counts(frame_data, main_camp)
        value = 0.0
        enemy_id = actor_id(enemy_hero)
        hit_enemy = any(
            get_any(hit, "hit_target", "runtime_id", default=0) == enemy_id
        ...
        target_enemy = get_any(main_hero, "attack_target", default=0) == enemy_id
        if enemy_visible:
            enemy_near_lane = bool(enemy_soldiers) and math.dist(enemy_pos, hero_pos) <= 14000.0
            if enemy_near_lane and (hit_enemy or target_enemy):
                value += 1.0
            if self.in_enemy_tower_range(main_hero, enemy_tower):
                value -= 0.6
        else:
            if enemy_soldiers and our_soldiers >= their_soldiers:
                value += 0.45
            if bool(get_any(main_hero, "is_in_grass", default=False)) and not enemy_soldiers:
                value -= 0.25
        return max(min(value, 1.2), -0.8)
    ...
    def calculate_early_bad_fight(self, frame_data, main_hero, enemy_hero, main_tower, enemy_tower):
        if not main_hero or not enemy_hero or hp_ratio(enemy_hero) <= 0:
            return 0.0
        frame_no = int(get_any(frame_data, "frame_no", "frameNo", default=0) or 0)
        level = int(get_any(main_hero, "level", default=1) or 1)
        if level >= getattr(GameConfig, "EARLY_LANE_DISCIPLINE_LEVEL", 4):
            return 0.0
        if frame_no > getattr(GameConfig, "EARLY_LANE_DISCIPLINE_FRAME", 2200):
            return 0.0
        if self.enemy_hit_or_targeting_us(main_hero, enemy_hero):
            return 0.0
        enemy_id = actor_id(enemy_hero)
        actively_fighting = (
            get_any(main_hero, "attack_target", default=0) == enemy_id
            or any(get_any(hit, "hit_target", "runtime_id", default=0) == enemy_id for hit in get_any(main_hero, "hit_target_info", default=[]) or [])
        )
        if not actively_fighting:
            return 0.0
        if not self.forward_of_lane_midpoint(main_hero, main_tower, enemy_tower):
            return 0.0
        money_gap = (
            float(get_any(main_hero, "money_cnt", "money", default=0) or 0)
            - float(get_any(enemy_hero, "money_cnt", "money", default=0) or 0)
        )
        level_gap = level - int(get_any(enemy_hero, "level", default=1) or 1)
        main_camp = camp_id(main_hero.get("camp"))
        our_soldiers, enemy_soldiers = self.lane_counts(frame_data, main_camp)
        lane_ok = our_soldiers >= enemy_soldiers + 1
        if money_gap >= 600 or level_gap >= 1 or lane_ok or hp_ratio(enemy_hero) <= 0.35:
            return 0.0
        if int(get_any(main_hero, "config_id", "configId", default=0) or 0) == 133 and int(get_any(enemy_hero, "config_id", "configId", default=0) or 0) == 112:
            return 1.35
        return 1.0

    def enemy_hit_or_targeting_us(self, main_hero, enemy_hero):
        main_id = actor_id(main_hero)
        if get_any(enemy_hero, "attack_target", default=0) == main_id:
            return True
        return any(get_any(hit, "hit_target", "runtime_id", default=0) == main_id for hit in get_any(enemy_hero, "hit_target_info", default=[]) or [])

    def forward_of_lane_midpoint(self, main_hero, main_tower, enemy_tower):
        if not main_hero or not main_tower or not enemy_tower:
            return False
        hx, hz = pos(main_hero)
        ox, oz = pos(main_tower)
        ex, ez = pos(enemy_tower)
        lx, lz = ex - ox, ez - oz
        denom = max(lx * lx + lz * lz, 1.0)
        progress = ((hx - ox) * lx + (hz - oz) * lz) / denom
        margin = getattr(GameConfig, "EARLY_CENTER_CROSS_MARGIN", 1500.0) / max(math.sqrt(denom), 1.0)
        return progress > 0.5 + margin
    ...
    def get_reward(self, frame_data, reward_dict):
        ...
        for reward_name, reward_struct in self.m_cur_calc_frame_map.items():
            if reward_name == "hp_point":
                ...
                reward_struct.value = self.zero_sum_current(reward_name)
            else:
                reward_struct.value = self.zero_sum_delta(reward_name)
            ...
            phase_scale_map = (
                getattr(GameConfig, "REWARD_LATE_SCALE", {})
                if frame_no >= getattr(GameConfig, "REWARD_LATE_PHASE_FRAME", 3800)
                else getattr(GameConfig, "REWARD_EARLY_SCALE", {})
            )
            phase_scale = float(phase_scale_map.get(reward_name, 1.0) or 1.0)
            reward_dict[reward_name] = reward_struct.value
            reward_dict[f"{reward_name}_origin"] = reward_struct.value
            reward_dict[f"{reward_name}_weight"] = reward_struct.value * reward_struct.weight * time_scale * phase_scale
    ...
    def zero_sum_current(self, reward_name):
        return self.m_main_calc_frame_map[reward_name].cur_frame_value - self.m_enemy_calc_frame_map[reward_name].cur_frame_value
