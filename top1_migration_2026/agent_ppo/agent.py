...
class Agent(BaseAgent):
    ...
    def action_process(self, observation, act_data, is_stochastic):
        ...
        disadvantage_action = self._fight_disadvantage_retreat_action(observation)
        if disadvantage_action is not None:
            if is_stochastic:
                act_data.action = disadvantage_action
            else:
                act_data.d_action = disadvantage_action
            return disadvantage_action
        ...
        guarded_action = self._guard_misused_skill_action(observation, action)
        if guarded_action is not None:
            if is_stochastic:
                act_data.action = guarded_action
            else:
                act_data.d_action = guarded_action
    ...
    def _combat_summoner_action(self, observation):
        ...
        if skill_id not in (
            game_config("BERSERK_SKILL_ID", 80110),
            game_config("STUN_SUMMONER_SKILL_ID", 80103),
        ):
            return None
        enemy = self._enemy_hero_state(observation)
        in_combat = self._can_fight_enemy_hero(observation) or self._in_hero_combat_window(observation, 45)
        if not in_combat:
            return None
        ...
        if skill_id == game_config("STUN_SUMMONER_SKILL_ID", 80103):
            if not enemy or self._enemy_dead(enemy) or self._is_unseen_actor(enemy):
                return None
            if math.dist(self._actor_pos(hero), self._actor_pos(enemy)) > game_config("STUN_SUMMONER_RANGE", 6000.0):
                return None
    ...
    def _fight_disadvantage_retreat_action(self, observation):
        ...
        combat_summoner = selected_summoner in (
            game_config("BERSERK_SKILL_ID", 80110),
            game_config("STUN_SUMMONER_SKILL_ID", 80103),
        )
        ...
        if our_tower is None:
            return None
        return self._move_to_point_action(legal_actions, self._actor_pos(hero), self._actor_pos(our_tower))
    ...
    def _force_leave_base_action(self, observation):
        ...
        return self._move_to_point_action(
            legal_actions, self._actor_pos(hero), game_config("MID_LANE_TARGET", (0.0, 0.0))
        )
    ...
    def _post_fight_macro_action(self, observation):
        ...
        if enemy_soldier is not None:
            clear_action = self._clear_lane_action(observation, legal_actions, hero, enemy_soldier)
    ...
    def _should_finish_push_after_fight(self, observation, hero, enemy):
        if int(hero.get("level", 1) or 1) < game_config("POST_FIGHT_PUSH_LEVEL", 9):
            return False
        if float(hero.get("money_cnt", hero.get("money", 0)) or 0) <= game_config("POST_FIGHT_PUSH_MONEY", 6000):
            return False
        if self._hp_ratio(hero) <= game_config("POST_FIGHT_PUSH_HP_THRESHOLD_LATE", 0.05):
            return False
    ...
    def _clear_lane_action(self, observation, legal_actions, hero, target):
    ...
    def _low_hp_rune_action(self, observation):
        ...
        if not hero or self._hp_ratio(hero) > game_config("RUNE_LOW_HP_THRESHOLD", 0.85):
            return None
        cake = self._nearest_cake(observation, hero)
        if cake is None:
            return None
    ...
    def _di_renjie_skill2_action(self, legal_actions, hero, enemy):
        if self._hero_config_id(hero) != 133:
            return None
        if not enemy or self._is_unseen_actor(enemy) or float(enemy.get("hp", 0) or 0) <= 0:
            return None
        button = 5
        if button >= len(legal_actions[0]) or legal_actions[0][button] <= 0:
            return None
    ...
    def _safe_tower_attack_action(self, observation, legal_actions):
        ...
        if legal_actions[0][button] <= 0:
            return None
        hero = self._main_hero_state(observation)
        money = float((hero or {}).get("money_cnt", (hero or {}).get("money", 0)) or 0)
        if money < game_config("TOWER_EARLY_PUSH_MONEY", 5500):
            return None
        target_legal = legal_actions[-1].reshape([self.legal_action_size[0], self.label_size_list[-1]])[button]
    ...
    def _tower_retreat_action(self, observation):
        ...
        hero_id = hero.get("runtime_id", hero.get("player_id"))
        tower_target = enemy_tower.get("attack_target", 0)
        if tower_target != hero_id and not self._unsafe_enemy_tower_zone(observation, hero, enemy_tower):
            return None
    ...
    def _update_combat_memory(self, observation):
        ...
        if hero.get("attack_target") == enemy_id or enemy.get("attack_target") == hero_id or hit_enemy or enemy_hit_us:
