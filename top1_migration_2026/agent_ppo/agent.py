...
class Agent(BaseAgent):
    def __init__(self, agent_type="player", device=None, logger=None, monitor=None):
        ...
        self.macro_intent = None
        self.macro_intent_until_frame = -100000
        self.last_move_grid = None
        self.last_move_frame = -100000
        self.move_flip_count = 0
        self.move_oscillation_lock_until = -100000
    ...
    def init_config(self, config_data):
        ...
        for hero_id in my_heroes:
            skill_cfg = GameConfig.HERO_SUMMONER_SKILL.get(hero_id, 80102)
    ...
    def action_process(self, observation, act_data, is_stochastic):
        ...
        self.current_frame_no = self._frame_no(observation)
        ...
        locked_action = self._locked_macro_intent_action(observation)
        if locked_action is not None:
            if is_stochastic:
                act_data.action = locked_action
            else:
                act_data.d_action = locked_action
        ...
        early_lane_action = self._early_lane_discipline_action(observation)
        if early_lane_action is not None:
            if is_stochastic:
                act_data.action = early_lane_action
            else:
                act_data.d_action = early_lane_action
            return early_lane_action
        ...
        luban_press_action = self._luban_vs_di_press_action(observation)
        if luban_press_action is not None:
            if is_stochastic:
                act_data.action = luban_press_action
            else:
                act_data.d_action = luban_press_action
            return luban_press_action
    ...
    def _frame_no(self, observation):
        return int(observation.get("frame_state", {}).get("frame_no", 0) or 0)

    def _set_macro_intent(self, intent, observation, frames):
        self.macro_intent = intent
        self.macro_intent_until_frame = self._frame_no(observation) + int(frames)

    def _clear_macro_intent(self, intent=None):
        if intent is None or self.macro_intent == intent:
            self.macro_intent = None
            self.macro_intent_until_frame = -100000

    def _locked_macro_intent_action(self, observation):
        if self.macro_intent is None or self._frame_no(observation) > self.macro_intent_until_frame:
            self._clear_macro_intent()
            return None

        hero = self._main_hero_state(observation)
        legal_actions = self._split_legal_actions(observation)
        if not hero or legal_actions is None:
            return None

        if self.macro_intent == "RETREAT_BASE":
            if self._enemy_soldiers_near_own_tower(observation, hero) and self._hp_ratio(hero) >= game_config("RECALL_DEFEND_EXIT_HP", 0.30):
                self._set_macro_intent("DEFEND_TOWER", observation, game_config("DEFEND_TOWER_LOCK_FRAMES", 45))
                return self._hold_own_tower_action(observation, legal_actions)
            self._clear_macro_intent("RETREAT_BASE")
            return None

        if self.macro_intent == "DEFEND_TOWER":
            if not self._enemy_soldiers_near_own_tower(observation, hero):
                self._clear_macro_intent("DEFEND_TOWER")
                return None
            return self._hold_own_tower_action(observation, legal_actions)

        if self.macro_intent == "ENEMY_RUNE_ESCAPE":
            return self._return_to_wave_or_mid_action(observation, legal_actions, hero)

        if self.macro_intent == "SAFE_PUSH":
            ...
            if tower is None:
                ...
                if enemy_tower is not None and self._enemy_dead(enemy) and self._wave_can_support_push(observation, hero, enemy):
                    return self._move_to_point_action(legal_actions, self._actor_pos(hero), self._actor_pos(enemy_tower))
                self._clear_macro_intent("SAFE_PUSH")
                return None
            action = self._safe_tower_attack_action(observation, legal_actions)
            if action is not None:
                return action
            return self._move_to_point_action(legal_actions, self._actor_pos(hero), self._actor_pos(tower))

        if self.macro_intent == "COMMIT_FIGHT":
            enemy = self._enemy_hero_state(observation)
            if enemy and self._should_commit_combat(observation, hero, enemy):
                action = self._hero_specific_combat_action(observation)
                if action is not None:
                    return action
            self._clear_macro_intent("COMMIT_FIGHT")
        return None

    def _return_to_wave_or_mid_action(self, observation, legal_actions, hero):
        target = None
        our_soldiers = self._soldiers_by_side(observation, hero, same_side=True)
        enemy_tower = self._nearest_enemy_tower(observation, hero)
        if enemy_tower is not None:
            tower_range = float(enemy_tower.get("attack_range", 0) or 0) + 2500.0
            our_soldiers = [
                soldier
                for soldier in our_soldiers
                if math.dist(self._actor_pos(soldier), self._actor_pos(enemy_tower)) > tower_range
            ]
        if our_soldiers:
            target = self._centroid(our_soldiers[:3])
        if target is None:
        ...
        if math.dist(self._actor_pos(hero), target) <= game_config("RETURN_WAVE_RADIUS", 4200.0):
            self._clear_macro_intent("ENEMY_RUNE_ESCAPE")
            return None
        return self._move_to_point_action(legal_actions, self._actor_pos(hero), target)

    def _enemy_tower_hero_forbid_action(self, observation):
        hero = self._main_hero_state(observation)
        enemy = self._enemy_hero_state(observation)
        if not hero or not enemy or self._enemy_dead(enemy) or self._is_unseen_actor(enemy):
            return None
        enemy_tower = self._nearest_enemy_tower(observation, hero)
        if enemy_tower is None or not self._in_enemy_tower_range(hero, enemy_tower):
            return None
        enemy_dist = math.dist(self._actor_pos(hero), self._actor_pos(enemy))
        threat_range = max(float(hero.get("attack_range", 0) or 0), float(enemy.get("attack_range", 0) or 0)) + 4500.0
        if enemy_dist > threat_range:
            return None
        if self._can_dive_enemy_under_tower(observation, hero, enemy, enemy_tower):
            return None
        legal_actions = self._split_legal_actions(observation)
        if legal_actions is None or legal_actions[0][2] <= 0:
            return None
        return self._retreat_from_enemy_tower_action(observation, legal_actions, hero, enemy_tower)

    def _retreat_from_enemy_tower_action(self, observation, legal_actions, hero, enemy_tower):
        ...
        if float(np.linalg.norm(away)) <= 1.0:
            our_tower = self._nearest_our_tower(observation, hero)
            target = self._actor_pos(our_tower) if our_tower is not None else game_config("MID_LANE_TARGET", (0.0, 0.0))
        else:
            target = (hero_pos + away / max(float(np.linalg.norm(away)), 1.0) * 10000.0).tolist()
        return self._move_to_point_action(legal_actions, self._actor_pos(hero), target)

    def _can_dive_enemy_under_tower(self, observation, hero, enemy, enemy_tower):
        hero_id = hero.get("runtime_id", hero.get("player_id"))
        if enemy_tower.get("attack_target", 0) == hero_id:
            return False
        score = self._fight_advantage_score(observation, hero, enemy)
        enemy_hp = self._hp_ratio(enemy)
        hero_hp = self._hp_ratio(hero)
        if hero_hp < game_config("ENEMY_TOWER_FORBID_HERO_HP", 0.45):
            return False
        if enemy_hp <= game_config("ENEMY_TOWER_DIVE_ENEMY_HP", 0.15) and score >= game_config("FIGHT_COMMIT_SCORE", 0.08):
            return True
        return score >= game_config("ENEMY_TOWER_DIVE_SCORE", 0.55) and enemy_hp <= 0.28

    def _is_late_game(self, observation):
        return self._frame_no(observation) >= game_config("LATE_GAME_FRAME", 3800)

    def _money_gap(self, hero, enemy):
        gap = float(hero.get("money_cnt", hero.get("money", 0)) or 0)
        if enemy:
            gap -= float(enemy.get("money_cnt", enemy.get("money", 0)) or 0)
        return gap

    def _level_gap(self, hero, enemy):
        gap = int(hero.get("level", 1) or 1)
        if enemy:
            gap -= int(enemy.get("level", 1) or 1)
        return gap

    def _advantage_state(self, observation, hero, enemy):
        return (
            self._money_gap(hero, enemy) >= game_config("ADVANTAGE_MONEY_GAP", 1000)
            or self._level_gap(hero, enemy) >= game_config("ADVANTAGE_LEVEL_GAP", 1)
            or self._wave_pressure_score(observation, hero) >= 0.50
        )

    def _enemy_lane_threat_to_our_tower(self, observation, hero):
        our_tower = self._nearest_our_tower(observation, hero)
        if our_tower is None:
            return False
        player_camp = camp_id(observation.get("camp", observation.get("player_camp", self.hero_camp)))
        threat_radius = game_config("RECALL_LANE_THREAT_RADIUS", 14000.0)
        enemy_soldiers = [
            npc
            ...
            and math.dist(self._actor_pos(npc), self._actor_pos(our_tower)) <= threat_radius
        ]
        return len(enemy_soldiers) >= 2

    def _has_sustain_resource(self, observation, hero):
        radius = game_config("SUSTAIN_RESOURCE_RADIUS", 26000.0)
        hero_pos = self._actor_pos(hero)
        cake = self._nearest_own_cake(observation, hero)
        if cake is not None and math.dist(hero_pos, self._actor_pos(cake)) <= radius:
            return True
        for npc in observation.get("frame_state", {}).get("npc_states", []) or []:
            if float(npc.get("hp", 0) or 0) <= 0 or self._is_unseen_actor(npc):
                continue
            if not (self._is_soldier_actor(npc) or self._is_monster_actor(npc)):
                continue
            if math.dist(hero_pos, self._actor_pos(npc)) <= radius:
                return True
        return False

    def _should_skip_recall_for_map(self, observation, hero, enemy, hp_rate):
        if hp_rate <= game_config("CRITICAL_HOME_HP_THRESHOLD", 0.16):
            return False
        if self._finish_game_window(observation, hero, enemy):
            return True
        if self._enemy_lane_threat_to_our_tower(observation, hero) and hp_rate >= game_config("ADVANTAGE_RECALL_HP", 0.18):
            return True
        if self._advantage_state(observation, hero, enemy) and hp_rate > game_config("ADVANTAGE_RECALL_HP", 0.18):
            return True
        if self._has_sustain_resource(observation, hero) and hp_rate >= game_config("ADVANTAGE_RECALL_HP", 0.18):
            return True
        if self._is_late_game(observation) and hp_rate > game_config("ENDGAME_RECALL_HP", 0.12):
            if self._enemy_dead(enemy) or self._advantage_state(observation, hero, enemy) or self._has_sustain_resource(observation, hero):
                return True
        return False

    def _early_bad_matchup(self, observation, hero, enemy):
        if self._frame_no(observation) > game_config("EARLY_BAD_MATCHUP_FRAME", 1800):
            return False
        return self._hero_config_id(hero) == 133 and enemy and self._hero_config_id(enemy) == 112

    def _luban_press_matchup(self, observation, hero, enemy):
        if self._frame_no(observation) > game_config("LUBAN_VS_DI_EARLY_FRAME", 1800):
            return False
        return self._hero_config_id(hero) == 112 and enemy and self._hero_config_id(enemy) == 133
    ...
    def _combat_summoner_action(self, observation):
        ...
        if self._fight_advantage_score(observation, hero, enemy) < game_config("FIGHT_RETALIATE_SCORE", -0.08):
            return None
        if self._hp_ratio(hero) > game_config("BERSERK_HP_THRESHOLD", 0.70):
            return None
        if frame_no - self.combat_start_frame < game_config("BERSERK_MIN_COMBAT_FRAMES", 60):
            return None
        if self._hp_ratio(hero) - self._hp_ratio(enemy) < -0.18 or self._hp_ratio(hero) < 0.22:
            return None
        legal_actions = self._split_legal_actions(observation)
        button = game_config("CHOSEN_SUMMONER_BUTTON", 8)
    ...
    def _fight_disadvantage_retreat_action(self, observation):
        ...
        enemy_hp = self._hp_ratio(enemy)
        score = self._fight_advantage_score(observation, hero, enemy)
        if self._should_skip_recall_for_map(observation, hero, enemy, hero_hp) and score >= -0.45:
            return None
        ...
        if math.dist(self._actor_pos(hero), target) <= 2500.0:
            return None
        self._set_macro_intent("RETREAT_BASE", observation, game_config("RETREAT_BASE_LOCK_FRAMES", 45))
        return self._move_to_point_action(legal_actions, self._actor_pos(hero), target)
    ...
    def _retaliate_when_attacked_action(self, observation):
        ...
        if enemy_tower is not None and self._in_enemy_tower_range(hero, enemy_tower):
            if not self._can_dive_enemy_under_tower(observation, hero, enemy, enemy_tower):
                return None
    ...
    def _early_lane_discipline_action(self, observation):
        hero = self._main_hero_state(observation)
        enemy = self._enemy_hero_state(observation)
        if not hero or not enemy or self._enemy_dead(enemy) or self._is_unseen_actor(enemy):
            return None
        if int(hero.get("level", 1) or 1) >= game_config("EARLY_LANE_DISCIPLINE_LEVEL", 4):
            return None
        if self._frame_no(observation) > game_config("EARLY_LANE_DISCIPLINE_FRAME", 2200):
            return None
        ...
        if self._can_finish_enemy_without_tower_risk(observation, hero, enemy):
            return None
        if not self._is_forward_of_lane_midpoint(observation, hero):
            return None

        score = self._fight_advantage_score(observation, hero, enemy)
        if score >= game_config("FIGHT_ADVANTAGE_ENGAGE_SCORE", 0.18):
            return None

        legal_actions = self._split_legal_actions(observation)
        if legal_actions is None:
            return None
        lane_target = self._nearest_enemy_soldier(observation, hero)
        if lane_target is not None and self._allied_soldiers_near(hero, observation):
            clear = self._clear_lane_action(observation, legal_actions, hero, lane_target)
            if clear is not None:
                return clear
        grass = self._own_side_grass_point(observation, hero)
        if grass is not None and legal_actions[0][2] > 0:
            return self._move_to_point_action(legal_actions, self._actor_pos(hero), grass)
        our_tower = self._nearest_our_tower(observation, hero)
        if our_tower is not None and legal_actions[0][2] > 0:
            return self._move_to_point_action(legal_actions, self._actor_pos(hero), self._own_tower_safe_anchor(hero, our_tower))
        return None

    def _is_forward_of_lane_midpoint(self, observation, hero):
        our_tower = self._nearest_our_tower(observation, hero)
        enemy_tower = self._nearest_enemy_tower(observation, hero)
        if our_tower is None or enemy_tower is None:
            hero_pos = self._actor_pos(hero)
            return math.dist(hero_pos, game_config("MID_LANE_TARGET", (0.0, 0.0))) <= 16000.0
        hero_pos = np.array(self._actor_pos(hero), dtype=np.float32)
        our_pos = np.array(self._actor_pos(our_tower), dtype=np.float32)
        enemy_pos = np.array(self._actor_pos(enemy_tower), dtype=np.float32)
        lane = enemy_pos - our_pos
        denom = max(float(np.dot(lane, lane)), 1.0)
        progress = float(np.dot(hero_pos - our_pos, lane) / denom)
        return progress > 0.5 + game_config("EARLY_CENTER_CROSS_MARGIN", 1500.0) / max(float(np.linalg.norm(lane)), 1.0)

    def _allied_soldiers_near(self, hero, observation):
        hero_pos = self._actor_pos(hero)
        return any(
            math.dist(hero_pos, self._actor_pos(soldier)) <= 11000.0
            for soldier in self._soldiers_by_side(observation, hero, same_side=True)
        )

    def _own_side_grass_point(self, observation, hero):
        ...
        hero_pos = self._actor_pos(hero)
        if our_tower is None or enemy_tower is None:
            return list(min(points, key=lambda point: math.dist(point, hero_pos)))
        our_pos = np.array(self._actor_pos(our_tower), dtype=np.float32)
        enemy_pos = np.array(self._actor_pos(enemy_tower), dtype=np.float32)
        lane = enemy_pos - our_pos
        denom = max(float(np.dot(lane, lane)), 1.0)
        ...
        def progress(point):
            return float(np.dot(np.array(point, dtype=np.float32) - our_pos, lane) / denom)

        candidates = [point for point in points if progress(point) <= 0.50]
        if not candidates:
            candidates = points
        return list(min(candidates, key=lambda point: math.dist(point, hero_pos) + 0.25 * math.dist(point, self._actor_pos(our_tower))))
    ...
    def _base_heal_lock_action(self, observation):
        ...
        if hp_rate >= game_config("BASE_HEAL_EXIT_HP", 0.98):
            return None
        legal_actions = self._split_legal_actions(observation)
        if legal_actions is None:
            return None
    ...
    def _should_commit_combat(self, observation, hero, enemy):
        ...
        enemy_tower = self._nearest_enemy_tower(observation, hero)
        if enemy_tower is not None and self._in_enemy_tower_range(hero, enemy_tower):
            return self._can_dive_enemy_under_tower(observation, hero, enemy, enemy_tower)
        if self._enemy_hit_or_targeting_us(hero, enemy):
        ...
        if self._can_fight_enemy_hero(observation):
            commit = self._fight_advantage_score(observation, hero, enemy) >= game_config("FIGHT_COMMIT_SCORE", 0.08)
            if commit:
                self._set_macro_intent("COMMIT_FIGHT", observation, game_config("COMMIT_FIGHT_LOCK_FRAMES", 30))
            return commit
        return False
    ...
    def _post_fight_macro_action(self, observation):
        ...
        post_kill_rune_window = enemy_dead and self._post_kill_rune_window(observation, hero, enemy)
        if post_kill_rune_window:
        ...
        if enemy_dead and self._should_finish_push_after_fight(observation, hero, enemy):
            self._set_macro_intent("SAFE_PUSH", observation, game_config("SAFE_PUSH_LOCK_FRAMES", 120))
        ...
        if enemy_dead and not post_kill_rune_window:
            monster_action = self._post_kill_monster_action(observation, legal_actions, hero)
            if monster_action is not None:
                return monster_action
    ...
    def _post_kill_rune_window(self, observation, hero, enemy):
        if self._enemy_soldiers_near_own_tower(observation, hero):
            return False
        return self._safe_enemy_tower_target(observation) is not None or self._wave_can_support_push(
    ...
    def _enemy_rune_after_kill_action(self, observation, legal_actions, hero, enemy):
        ...
        if legal_actions[0][2] <= 0:
            return None
        if dist <= game_config("ENEMY_RUNE_ESCAPE_PICK_RADIUS", 6200.0):
            self._set_macro_intent("ENEMY_RUNE_ESCAPE", observation, game_config("ENEMY_RUNE_ESCAPE_FRAMES", 90))
            return self._return_to_wave_or_mid_action(observation, legal_actions, hero)
        return self._move_to_point_action(legal_actions, self._actor_pos(hero), self._actor_pos(cake))

    def _post_kill_monster_action(self, observation, legal_actions, hero):
        money = float(hero.get("money_cnt", hero.get("money", 0)) or 0)
        if money > game_config("POST_KILL_MONSTER_MAX_MONEY", 5200):
            return None
        if self._enemy_soldiers_near_own_tower(observation, hero):
            return None
        ...
        if monster is None:
            return None
        if math.dist(self._actor_pos(hero), self._actor_pos(monster)) > game_config("POST_KILL_MONSTER_RADIUS", 30000.0):
            return None
        frame_no = self._frame_no(observation)
        self.focus_monster_id = int(monster.get("runtime_id", monster.get("player_id", 0)) or 0)
        self.focus_monster_until_frame = frame_no + 150
        target_index = self._target_index_for_actor(observation, monster)
        skill = self._legal_directed_action_with_actor(legal_actions, 4, hero, monster, target_index)
        ...
        if attack is not None:
            return attack
        if legal_actions[0][2] > 0:
            return self._move_to_point_action(legal_actions, self._actor_pos(hero), self._actor_pos(monster))
        return None
    ...
    def _should_finish_push_after_fight(self, observation, hero, enemy):
        hp_rate = self._hp_ratio(hero)
        if hp_rate <= game_config("SUPPLY_RETURN_HOME_HP_THRESHOLD", 0.20) and not (
            self._is_late_game(observation) or self._advantage_state(observation, hero, enemy)
        ):
            return False
    ...
    def _should_recall_after_fight(self, observation, hero, enemy):
        hp_rate = self._hp_ratio(hero)
        if self._should_skip_recall_for_map(observation, hero, enemy, hp_rate):
            return False
    ...
    def _safe_recall_action(self, observation):
        ...
        if not hero:
            return None
        hp_rate = self._hp_ratio(hero)
        if hp_rate >= game_config("RECALL_EXIT_HP", 0.45):
            self._clear_macro_intent("RETREAT_BASE")
            return None
        enemy = self._enemy_hero_state(observation)
        if self._should_skip_recall_for_map(observation, hero, enemy, hp_rate):
        ...
        if our_tower is None:
            return None
        if self._enemy_soldiers_near_own_tower(observation, hero):
            if hp_rate >= game_config("RECALL_DEFEND_EXIT_HP", 0.30):
                self._set_macro_intent("DEFEND_TOWER", observation, game_config("DEFEND_TOWER_LOCK_FRAMES", 45))
            target = self._nearest_enemy_soldier(observation, hero)
    ...
    def _luban_vs_di_press_action(self, observation):
        hero = self._main_hero_state(observation)
        enemy = self._enemy_hero_state(observation)
        if not hero or not self._luban_press_matchup(observation, hero, enemy):
            return None
        if self._enemy_soldiers_near_own_tower(observation, hero):
            return None
        enemy_tower = self._nearest_enemy_tower(observation, hero)
        ...
        if enemy and not self._enemy_dead(enemy) and not self._is_unseen_actor(enemy):
            if self._enemy_in_attack_or_skill_range(hero, enemy) or self._enemy_hit_or_targeting_us(hero, enemy):
                action = self._legal_directed_action_flexible_target(legal_actions, 4, hero, enemy, (1, 0, 2))
                if action is not None:
                    return action
                attack = self._legal_action_with_target(legal_actions, 3, 1, strict_target=True)
                if attack is not None:
                    return attack
            if self._enemy_under_enemy_tower(observation, hero, enemy):
                return None

        lane_target = self._nearest_enemy_soldier(observation, hero)
        if lane_target is not None:
            lane_dist = math.dist(self._actor_pos(hero), self._actor_pos(lane_target))
            if int(hero.get("level", 1) or 1) <= 1 or lane_dist <= game_config("LUBAN_PRESS_IGNORE_LANE_RADIUS", 7600.0):
                return None

        probe = self._luban_probe_grass_action(observation, legal_actions, hero, enemy)
        if probe is not None:
            return probe

        grass = self._enemy_side_grass_point(observation, hero)
        if grass is None or legal_actions[0][2] <= 0:
            return None
        if math.dist(self._actor_pos(hero), grass) > game_config("LUBAN_PRESS_GRASS_RADIUS", 22000.0):
            return None
        self.grass_intent_until_frame = self._frame_no(observation) + game_config("GRASS_AMBUSH_EXIT_FRAMES", 90)
        return self._move_to_point_action(legal_actions, self._actor_pos(hero), grass)

    def _luban_probe_grass_action(self, observation, legal_actions, hero, enemy):
        if int(hero.get("level", 1) or 1) < game_config("LUBAN_PRESS_GRASS_MIN_LEVEL", 2):
            return None
        if enemy and not self._enemy_dead(enemy) and not self._is_unseen_actor(enemy):
            return None
        if not enemy or self._enemy_dead(enemy):
            return None
        frame_no = self._frame_no(observation)
        if frame_no - self.last_enemy_alive_frame > game_config("LUBAN_PROBE_GRASS_FRAMES_AFTER_LOST", 180):
            return None
        target = self._suspected_enemy_grass_point(observation, hero)
        if target is None:
            return None
        if math.dist(self._actor_pos(hero), target) > game_config("LUBAN_PROBE_GRASS_RANGE", 24000.0):
            return None
        return self._legal_directed_action_to_point(legal_actions, 5, hero, target, (0, 2))

    def _enemy_under_enemy_tower(self, observation, hero, enemy):
        tower = self._nearest_enemy_tower(observation, hero)
        if tower is None or not enemy:
            return False
        return math.dist(self._actor_pos(enemy), self._actor_pos(tower)) <= float(tower.get("attack_range", 0) or 0) + 2000.0

    def _enemy_side_grass_point(self, observation, hero):
        enemy_tower = self._nearest_enemy_tower(observation, hero)
        points = game_config("GRASS_POINTS", [])
        if not points:
            return None
        ...
        tower_pos = self._actor_pos(enemy_tower)
        safe_points = [
            point
            for point in points
            if math.dist(point, tower_pos) > float(enemy_tower.get("attack_range", 0) or 0) + 2500.0
        ]
        if not safe_points:
            safe_points = points
        return list(min(safe_points, key=lambda point: math.dist(point, tower_pos) + 0.35 * math.dist(point, hero_pos)))

    def _suspected_enemy_grass_point(self, observation, hero):
        points = game_config("GRASS_POINTS", [])
        if not points:
            return None
        reference = self.last_enemy_seen_pos or self._actor_pos(hero)
        return list(min(points, key=lambda point: math.dist(point, reference)))
    ...
    def _fight_advantage_score(self, observation, hero, enemy):
        ...
        if self._summoner_skill_ready(hero):
            ...
            if skill_id == game_config("BERSERK_SKILL_ID", 80110):
                score += 0.16
            elif skill_id == game_config("HEAL_SUMMONER_SKILL_ID", 80102) and hero_hp <= game_config("LOW_HP_HEAL_THRESHOLD", 0.70):
                score += 0.08
        ...
        if hero_hp < 0.25:
            low_hp_penalty = 0.35
            if self._is_late_game(observation) or money_gap >= game_config("ADVANTAGE_MONEY_GAP", 1000):
                low_hp_penalty *= 0.55
            score -= low_hp_penalty
        elif hero_hp >= game_config("FIGHT_MIN_HEALTHY_HP", 0.38) and money_gap > 1000:
            score += 0.15
        if self._is_late_game(observation) and (money_gap >= 600 or level_gap >= 1):
            score += 0.12
        if self._has_sustain_resource(observation, hero) and hero_hp >= game_config("ADVANTAGE_RECALL_HP", 0.18):
            score += 0.08
        if self._early_bad_matchup(observation, hero, enemy) and not self._enemy_hit_or_targeting_us(hero, enemy):
            score -= game_config("DI_VS_LUBAN_EARLY_SCORE_PENALTY", 0.22)
        score += max(min(self.recent_trade_score, 1.0), -1.0) * 0.22
        if int(hero.get("level", 1) or 1) < game_config("EARLY_LANE_DISCIPLINE_LEVEL", 4) and self._is_forward_of_lane_midpoint(observation, hero):
            if enemy and not self._enemy_hit_or_targeting_us(hero, enemy):
                score -= 0.18
        return score
    ...
    def _legal_directed_action_to_point(self, legal_actions, button, hero, target_point, target_candidates):
        if hero is None or target_point is None:
            return None
        if button >= len(legal_actions[0]) or legal_actions[0][button] <= 0:
            return None

        action = [button]
        move_x, move_z = self._delta_action_16x16(self._actor_pos_for_action(hero), self._point_for_action(target_point))
        for index in range(1, len(self.label_size_list) - 1):
            legal = legal_actions[index]
            preferred = move_x if index == 3 else move_z if index == 4 else 0
            if preferred >= len(legal) or legal[preferred] <= 0:
    ...
    def _move_to_grid_action(self, legal_actions, move_x, move_z):
        ...
        move_x, move_z = self._stabilize_move_grid(legal_actions, move_x, move_z)
        return [2, move_x, move_z, 0, 0, 0]

    def _stabilize_move_grid(self, legal_actions, move_x, move_z):
        frame_no = int(getattr(self, "current_frame_no", 0) or 0)
        last = self.last_move_grid
        if last is not None and frame_no <= self.move_oscillation_lock_until:
            last_x, last_z = last
            if legal_actions[1][last_x] > 0 and legal_actions[2][last_z] > 0:
                return last_x, last_z

        if last is not None and frame_no - self.last_move_frame <= game_config("MOVE_OSCILLATION_WINDOW", 36):
            last_x, last_z = last
            opposite_x = abs((move_x - 8) + (last_x - 8)) <= 1 and abs(move_x - last_x) >= 6
            opposite_z = abs((move_z - 8) + (last_z - 8)) <= 1 and abs(move_z - last_z) >= 6
            if opposite_x or opposite_z:
                self.move_flip_count += 1
            else:
                self.move_flip_count = max(0, self.move_flip_count - 1)
            if self.move_flip_count >= game_config("MOVE_OSCILLATION_LIMIT", 3):
                self.move_oscillation_lock_until = frame_no + game_config("MOVE_OSCILLATION_LOCK_FRAMES", 45)
                self.move_flip_count = 0
                if legal_actions[1][last_x] > 0 and legal_actions[2][last_z] > 0:
                    move_x, move_z = last_x, last_z
        else:
            self.move_flip_count = 0

        self.last_move_grid = (move_x, move_z)
        self.last_move_frame = frame_no
    ...
    def _update_combat_memory(self, observation):
        ...
        if enemy:
            if self._enemy_dead(enemy):
                ...
                self.last_enemy_seen_pos = self._actor_pos(enemy)
        if not hero or not enemy:
            return
        hurt_to_hero = float(hero.get("total_hurt_to_hero", 0) or 0)
        be_hurt_by_hero = float(hero.get("total_be_hurt_by_hero", 0) or 0)
        hurt_delta = max(hurt_to_hero - self.last_total_hurt_to_hero, 0.0)
        be_hurt_delta = max(be_hurt_by_hero - self.last_total_be_hurt_by_hero, 0.0)
        self.last_total_hurt_to_hero = hurt_to_hero
        self.last_total_be_hurt_by_hero = be_hurt_by_hero
        if hurt_delta > 0 or be_hurt_delta > 0:
            trade_delta = (hurt_delta - be_hurt_delta) / max(game_config("EARLY_TRADE_MEMORY_SCALE", 1500.0), 1.0)
            self.recent_trade_score = max(min(self.recent_trade_score * 0.82 + trade_delta, 1.0), -1.0)
            self.recent_trade_frame = frame_no
        elif frame_no - self.recent_trade_frame > 180:
            self.recent_trade_score *= 0.96
        enemy_id = enemy.get("runtime_id", enemy.get("player_id"))
        hero_id = hero.get("runtime_id", hero.get("player_id"))
    ...
    def _can_fight_enemy_hero(self, observation):
        ...
        if score >= game_config("FIGHT_ADVANTAGE_STRONG_SCORE", 0.45):
            hero_range += 2500.0
        min_score = -0.25
        if self._early_bad_matchup(observation, hero, enemy):
            min_score = game_config("FIGHT_COMMIT_SCORE", 0.08)
            hero_range -= 1200.0
        if self._is_late_game(observation) or self._advantage_state(observation, hero, enemy):
            min_score -= 0.10
        return math.dist(self._actor_pos(hero), self._actor_pos(enemy)) <= hero_range and score >= min_score
